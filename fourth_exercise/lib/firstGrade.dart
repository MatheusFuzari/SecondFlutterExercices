...
import 'package:fourth_exercise/secoundGrade.dart';

class FirstGrade extends StatelessWidget {
  const FirstGrade({super.key, required this.studentName});
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: FirstGradePage(name: studentName),
    );
  }
}

class FirstGradePage extends StatefulWidget {
  const FirstGradePage({super.key, required this.name});
  final String name;
  @override
  State<FirstGradePage> createState() => _FirstGradePage();
}

class _FirstGradePage extends State<FirstGradePage> {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => SecondGrade(studentName: widget.name),
                  ));
            },
