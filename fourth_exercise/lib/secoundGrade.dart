...
import 'package:fourth_exercise/thirdGrade.dart';

class SecondGrade extends StatelessWidget {
  const SecondGrade({super.key, required this.studentName});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: SecondGradePage(name: studentName),
    );
  }
}

class SecondGradePage extends StatefulWidget {
  const SecondGradePage({super.key, required this.name});
  final String name;
  @override
  State<SecondGradePage> createState() => _SecondGradePage();
}

class _SecondGradePage extends State<SecondGradePage> {
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ThirdGrade(studentName: widget.name),
                  ));
            },
