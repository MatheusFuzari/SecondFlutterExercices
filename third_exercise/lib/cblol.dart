...
import 'package:third_exercise/valorant.dart';

void main() {
  runApp(const cblol());
}

class cblol extends StatelessWidget {
  const cblol({super.key});
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const cblolPage(),
    );
  }
}

class cblolPage extends StatefulWidget {
  const cblolPage({super.key});

  @override
  State<cblolPage> createState() => _cblolPage();
}

class _cblolPage extends State<cblolPage> {
  final List<String> _drivers = ["Tinowns", "TitaN", "MicaO", "CarioK"];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => valorant()));
        },
        child: Text("Next"),
      ),
    );
